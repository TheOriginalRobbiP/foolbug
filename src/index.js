...
client.on('interactionCreate', async (interaction) => {
  ...
  if (
    ...
    interaction.customId === 'bug_platform'
  ) {
  ...
  if (
    interaction.isStringSelectMenu() &&
    interaction.customId.startsWith('bug_severity__')
  ) {
    ...
    const whatExpected = new TextInputBuilder()
    ...
    const sceneName = new TextInputBuilder()
    ...
    const stepsToRepro = new TextInputBuilder()
      .setCustomId('steps')
    ...
    modal.addComponents(
      ...
      new ActionRowBuilder().addComponents(whatExpected),
      new ActionRowBuilder().addComponents(sceneName),
      new ActionRowBuilder().addComponents(stepsToRepro),
    );
  ...
  if (
    ...
    if (!bugChannel?.isTextBased()) {
      await interaction.reply({ content: '❌ Bug reports channel not found. Ask the server admin to check the BUG_CHANNEL_ID config.', ephemeral: true });
      return;
    }

    const report = await bugChannel.send({ embeds: [embed] });

    // Auto-create a thread for screenshots and discussion
